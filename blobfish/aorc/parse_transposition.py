...
class JSONLDConstructor:
    ...
    def populate_datasets(self) -> None:
        ...
        for obj_key in self.__get_key():
            dataset = DatasetConstructor(self.bucket, obj_key, self.script_path, self.docker_path, self.contact_name, self.contact_email, self.client)
...
class DatasetConstructor:
    def __init__(self, bucket: str, key: str, script_path: str, docker_path: str, contact_name: str, contact_email, client: Any | None) -> None:
        self.bucket = bucket
        self.key = key
        self.client = client
    ...
    def __find_source_zarrs(self) -> list[dict]:
        ...
        logging.info("Finding AORC precipitation files associated with model")
        sources = []
    ...
    def create(self) -> dict:
        ...
        dataset = {}
        dataset["title"] = f"Storm Transposition Model, {self.documentation.metadata.watershed_name} {self.documentation.format_start_date().strftime('%Y-%m-%d')}"
        dataset["description"] = f"Data documenting modeling of transposition region for {self.documentation.metadata.watershed_name}, {self.documentation.format_start_date().strftime('%Y-%m-%d')}"
        dataset["keyword"] = [
                "aorc",
                "transposition",
                "Indian Creek",
        ...
        dataset["rights"] = "This dataset is located on an s3 bucket which requires an AWS account which has been granted access to said bucket"
        ...
        dataset["image"] = {
                "@type": "dcat:Dataset",
                "title": f"Storm Transposition Model, {self.documentation.metadata.watershed_name} {self.documentation.format_start_date().strftime('%Y-%m-%d')}",
                ...
                "distribution": [
                    {
                        "@type": "dcat:Distribution",
                        "description": f"PNG file for {self.documentation.metadata.watershed_name} {self.documentation.format_start_date().strftime('%Y-%m-%d')} model",
